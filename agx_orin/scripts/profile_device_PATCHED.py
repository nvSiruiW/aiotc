...
class PowerSampler:
    ...
    Jetson note (adapted per JETSON_MANUAL.md troubleshooting, AGX Orin devkit):
    the original code called check_output(["tegrastats",...], timeout=0.5), but
    tegrastats streams forever and never exits, so it always raised TimeoutExpired
    and captured nothing (power came back blank). Here we launch ONE persistent
    tegrastats process and parse each streamed line. Rail preference: the board
    input rail (VDD_IN / VIN_SYS_5V0 / POM_5V_IN) = whole-board power; if none is
    present we sum all *mW rails as a fallback. This board reports VIN_SYS_5V0.
    """
    import re as _re
    _MW = _re.compile(r'([A-Za-z0-9_]+)\s+(\d+)mW')
    _BOARD_RAILS = ("VDD_IN", "VIN_SYS_5V0", "POM_5V_IN")
    def __init__(self, backend):
        self.backend, self.samples, self._stop = backend, [], threading.Event()
        self.proc = None
    def _parse_tegrastats(self, line):
        d = {k: int(v) for k, v in self._MW.findall(line)}
        if not d:
            return None
        for k in self._BOARD_RAILS:      # prefer whole-board input rail
            if k in d:
                return d[k] / 1000.0
        return sum(d.values()) / 1000.0  # fallback: sum reported rails
    def _read(self):  # nvidia-smi (card) path, unchanged
        try:
